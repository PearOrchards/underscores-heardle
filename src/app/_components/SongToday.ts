...
export async function SongToday(artist: string): Promise<SongData> {
  if (!process.env.HARDLY_RANDOM_SEED)
    throw new Error("Environment not correctly set up.");
  ...
  const totalSongs: number =
    songList.soundcloud.length + songList.tracker.length;

  const now = new Date();
  const daysSinceEpoch = Math.floor(now.getTime() / 8.64e7); // idx for today

  const shuffleRound = Math.floor(daysSinceEpoch / totalSongs);
  const shuffledList = shuffleArray(
    [...songList.soundcloud, ...songList.tracker],
    seed + shuffleRound,
  );

  const todayPick = shuffledList[daysSinceEpoch % totalSongs];
  const source =
    songList.soundcloud.indexOf(todayPick) > -1 ? "soundcloud" : "tracker";
