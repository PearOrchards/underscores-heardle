  ...
  return (
    <section>
      ...
      <div className={styles.track}>
        <div className={styles.innerTrack} data-cy="innerTrack">
          {!complete
            ? [0, 1, 2, 3, 4, 5].map((i) => {
                return (
                  <div
                    className={`${styles.block} ${currentAttempt >= i ? styles.active : ""}`}
                    key={i}
                  ></div>
                );
              })
        ...
        <div
          className={`${styles.progress} ${playing ? styles.progressPlay : ""}`}
          style={{
      ...
      {error ? (
        <>
          ...
          <Dialog isOpen={modalOpen} onClose={closeErrorModal}>
            <h2>whoops</h2>
            <p>
              Since something has gone wrong, we&apos;ll just give you the
              answer <br />
              It&apos;s &quot;{song}&quot;
            </p>
          </Dialog>
