...
export default function Input({
  ...
  const keyPress = (ev: React.KeyboardEvent<HTMLInputElement>) => {
    if (ev.key === "ArrowUp" || ev.key === "ArrowDown") {
      ...
      if (suggestions) {
        const oldSelected = suggestionsRef.current?.querySelector(
          `.${styles.selected}`,
        );
        let newSelected;
        if (ev.key == "ArrowUp") {
          const index = oldSelected
            ? Array.from(suggestions).indexOf(
                oldSelected as HTMLParagraphElement,
              )
            : suggestions.length;
  ...
  return !complete ? (
    <section>
      ...
      >
        <div
          className={`${styles.suggestions} ${showSuggestions ? styles.suggestionsShow : ""}`}
          ref={suggestionsRef}
        >
          {suggestions.slice(0, 9).map((s) => (
            <p onMouseEnter={hover} onMouseDown={() => autocomplete(s)} key={s}>
              {s}
            </p>
          ))}
        </div>
      ...
      <div className={styles.buttonRow}>
        <button onClick={skip} data-cy="skipBtn">
          SKIP {currentAttempt < 5 ? `(+${Math.pow(2, currentAttempt)}s)` : ""}
        </button>
        <button onClick={submitGuess} data-cy="submitBtn">
          SUBMIT
        </button>
      </div>
    </section>
