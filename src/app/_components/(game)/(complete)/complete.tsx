...
export default function Complete({
  ...
  for (let i = 0; i < 6; i++) {
    ...
    bars.push(
      <div
        key={i + 6}
        className={`${styles.attemptBlock} ${secondaryClass}`}
      ></div>,
    );
  }
  ...
  return (
    <section className={styles.complete} data-cy="complete">
      <div className={styles.topBox}>
        ...
        >
          go ({songData?.source}) <FontAwesomeIcon icon={faChevronRight} />
        </Link>
      </div>
      <div className={styles.middleSection}>
        <h1>{completionMessages[guesses.length - 1]}</h1>
        <div className={styles.barsHolder}>{bars}</div>
        <p>
          {guesses.length < 7
        ...
        <Dialog isOpen={modalOpen} onClose={closeCopyModal}>
          ...
          <p>
            turns out I don&apos;t have access to your clipboard! so... could
            you just copy the stuff below? sorry :3
          </p>
          <pre>{manualCopy}</pre>
        </Dialog>
